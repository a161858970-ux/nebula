...
## 2026-08-16 — 原型定稿并整合：球→胶囊→窗口一体化 Dock + silent-otter 开关

**改动**

- 左右 Dock 按定稿原型重构为「单元素自膨胀」：球本身就是胶囊，悬停宽度弹性展开（文字从 0 生长 + 小跳字），开窗后胶囊本体透明、窗口背景上移覆盖整个胶囊区域，整卡一个连续品牌色渐变表面，四角圆角 = 胶囊边缘圆的四分之一。
- 开窗时窗口自动上移置顶（dock top 18vh→8vh，左 Dock 其余球距收拢到 60%），左 Dock 整组严格垂直居中（顶距=底距）；窗口内内容单层滚动（暗系滚动条），歌单吸顶头/播放/回顶保持主项目原样。
- 箭头状态机：球态隐藏 → 胶囊态转下 ⌄ → 窗口态转上 ⌃（弹簧旋转）。
- 登录胶囊五平台等距圆点（列位与窗口垂直胶囊一一对应）+ 徽章上浮；登录窗口五个圆点各自错峰向下膨胀成垂直胶囊（图标成为胶囊顶盖）。
- 右栏设置胶囊箭头与登录一致贴左；歌词取色「自定义」按四字宽度居中对齐。
- 开关替换为 silent-otter-72 复刻：勾号独立滑动动感（right 0.3s vs 圆钮 transform 0.25s）、圆钮/勾号 em 比例缩放（设置内小号不突兀）、垂直对称无掉底、边缘灰调 #9a9aa3。
- 果冻弹性统一走 `--jelly-cap/--jelly-win` 变量（默认 1.32 / 1.28）。
- 歌单列表 hover 与背景三方案 hover 保持主项目原实现，未随原型改动。

**验证**

- pnpm build / qa / build:desktop 全绿；无头 Chrome 合并冒烟（球→胶囊变形/窗口覆盖无接缝/垂直居中 72=72/歌单展开吸顶/登录五球垂直胶囊/设置箭头贴左/小号开关比例）全通过、零控制台错误；已提交并推送 GitHub。
