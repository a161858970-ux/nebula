...
## 2026-09-13 — 新增项目 GitHub README（开发期版）+ 冷启动预热刷新

**目标**

1. GitHub 仓库此前只有源码、没有项目 README；先补一份开发期展示 README，后续随进度更新，最终版在 CI/CD 阶段定稿。
2. 换模型后按 `docs/meta/CONTEXT_SNAPSHOT.md` 协议完成冷启动预热（全量 docs 精读）。

**改动**

- 新增仓库根 `README.md`：项目定位与理念、当前状态表、功能一览（播放与队列 / 歌词系统 / 内容浏览 / 背景氛围 / 界面设置）、**平台支持矩阵**（登录 / 歌单 / 取链 / 歌词 / 评论 / 歌手页 / 专辑页）、Z0–Z4 界面层级、技术架构与目录结构、快速开始（含 `SPOTIFY_CLIENT_ID`、`pnpm build` 产物在仓库外等注意事项）、质量保障命令、文档索引、已知限制、路线图、免责声明、License 待定。
  - 准确性校正：队列面板与「下一首播放 FIFO」属 UI 分支 P1 成果，README 中标注"开发中、未合入 main"，避免与 main 现状不符。
- `docs/project/NOTES.md`：修正 4 处过时项（酷狗/汽水登录已实现、收藏功能已废弃、播放队列 UI 已完成）。
- `docs/meta/CONTEXT_SNAPSHOT.md`：刷新当前状态（两仓库 HEAD、P1 待复验、P2 目标、挂起决策），新增「模型迭代与连续性」小节记录本次模型更替与冷启动协议。
- 新增仓库根 `AGENTS.md`：Codex 自动加载的上下文恢复入口 + 硬性约定（QA 门禁、冻结文档真源、hover 规范、分支纪律）。

**验证**

- 文档类改动，不涉及构建 / QA；已核对 README 中的命令、路径、平台能力矩阵与代码实现一致（各适配器方法清单、`.gitignore` 已排除 `qq-cookie.txt` / `.env`）。

**备注**

- 播放器重构（Player P1 已实现、P2–P8 待开发）与全局 UI 底层重构均未在本条目范围内；P1 结果在分支 `ui/nebula-design-deepseek-codex`。
...
此前误判为渲染逻辑问题并加了增量渲染；用户反馈其它平台 1000+ 首全量渲染正常、仅酷狗 1370 首卡死，且要求恢复全量列表体验。用真实酷狗 1370 首数据在 Playwright 复现并定位。

**排查与结论**

- 真实数据校验：1370 首无重复 id/坏时长/异常字段，封面 1347 个在快速 CDN（imge.kugou.com）；后端拉取 2s 正常。
- Playwright 复现：全量渲染 1370 行在无头环境 3s 内完成、主线程恢复 8ms——但渲染期间 React 同步 commit 阻塞主线程；用户真实环境（GPU + 千张封面解码）阻塞放大，酷狗 1370 首越过 Windows 5s「未响应」阈值，而其它平台 1000-1100 首在临界以下。
- **根因：浏览器对千行 DOM 的同步布局/绘制，非数据或渲染逻辑问题。**

**修复**

- `styles.css`：`.pl-song` 加 `content-visibility: auto; contain-intrinsic-size: auto 44px`——DOM 全量保留（滚动条/查找/体验不变），浏览器只渲染视口内行、滚动按需渲染。
- 回滚增量渲染，恢复全量列表。
- 实测：1370 行渲染 42ms（此前秒级阻塞）、主线程 rAF 27ms。

**验证**

- tsc / build / 前端 qa 全绿；Playwright 复现脚本（真实 1370 首）渲染 42ms。

## 2026-08-26 — 酷狗大歌单导入卡死修复 + 兜底候选防翻唱保护

**目标**

1. 酷狗歌单（可达 1300+ 首）点击导入后 Electron 无响应——根因是 PlaylistDock 展开歌曲列表时一次性渲染全部歌曲行，渲染进程主线程阻塞。
2. 兜底搜索可能命中翻唱/Remix：无严格对照保护。

**改动**

- `PlaylistDock.tsx`：歌曲列表改为增量渲染（默认 60 行，滚动到底每次 +120），`songs` 变化自动重置；大歌单不再卡死。
- `SongResolver`：新增 `filterCandidate` 防翻唱保护——标题归一化完全相等优先；时长差 >8s 且 >10% 直接剔除；时长 ≤2s 视为严格匹配；歌手包含校验加分；候选排序按（精确命中 > 歌手+时长命中 > 相似）再按分数。

**验证**

- tsc / build / qa:backend（26/26）/ 前端 qa 全绿；酷狗歌单列表与详情接口实测（7 歌单 / 313ms）。

**遗留**

- 酷狗「收藏型歌单」（`collection_*` id）H5 分页拉取为空后回退 mobile 也失败——属正常报错路径；若用户常用收藏歌单可后续补专用接口。
